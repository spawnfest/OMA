...
-export([string/1,string/2,token/2,token/3,token/4,tokens/2,tokens/3, tokens/4]).
...
string(String, Line) -> string(String, Line, 1, String, []).

%% string(InChars) ->
%% string(InChars, L0) ->
%% string(InChars, L0, C0, TokenChars, Tokens) ->
%% {ok,Tokens,EndLoc} | {error,ErrorInfo,EndLoc}.
%% Note the line number going into yystate, L0, is line of token
%% start while line number returned is line of token end. We want line
%% of token start. EndLoc can be either TokenLine or {TokenLine, TokenCol}
%% depending on the 'error_location' option.
...
%% token(Continuation, Chars, Line, Col) ->
...
token(Cont, Chars, Line) -> token(Cont, Chars, Line, 1).
...
%% tokens(Continuation, Chars, Line, Col) ->
%% {more,Continuation} | {done,ReturnVal,RestChars}.
%% Must be careful when re-entering to append the latest characters to the
%% after characters in an accept. The continuation is:
...
tokens(Cont, Chars, Line) -> tokens(Cont, Chars, Line, 1).
